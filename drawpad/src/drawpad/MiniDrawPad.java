                   ...
                   public void actionPerformed(ActionEvent e)
                    {
                     JOptionPane.showMessageDialog(null,
                        "This is a mini drawing pad!\nBy Jian Yongkang 0808010208 ", " 画图板程序说明 ",JOptionPane.INFORMATION_MESSAGE );
                     }
                   }
              );
...
//导入图形图标，这些图标都存放在与源文件相同的目录下面
    for(int i=0;i<choices.length;i++)
    {
     items[i]=new ImageIcon("img\\"+names[i]+".jpg");
...
//新建一个画图基本单元对象的程序段
 void createNewItem()
  { if(currentChoice==14)//进行相应的游标设置
          ...
          else       drawingArea.setCursor(Cursor.getPredefinedCursor(Cursor.CROSSHAIR_CURSOR));
